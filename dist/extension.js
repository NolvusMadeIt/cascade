"use strict";var Z=Object.create;var H=Object.defineProperty;var ee=Object.getOwnPropertyDescriptor;var te=Object.getOwnPropertyNames;var se=Object.getPrototypeOf,ie=Object.prototype.hasOwnProperty;var ae=(o,e)=>{for(var t in e)H(o,t,{get:e[t],enumerable:!0})},_=(o,e,t,s)=>{if(e&&typeof e=="object"||typeof e=="function")for(let a of te(e))!ie.call(o,a)&&a!==t&&H(o,a,{get:()=>e[a],enumerable:!(s=ee(e,a))||s.enumerable});return o};var oe=(o,e,t)=>(t=o!=null?Z(se(o)):{},_(e||!o||!o.__esModule?H(t,"default",{value:o,enumerable:!0}):t,o)),ne=o=>_(H({},"__esModule",{value:!0}),o);var ye={};ae(ye,{activate:()=>be,deactivate:()=>we});module.exports=ne(ye);var i=oe(require("vscode"));var ce=80,G=200,z="cascade.sessions",Y="cascade.activeSession";function de(){return Date.now().toString(36)+Math.random().toString(36).slice(2,7)}function re(o){let e=o.replace(/```[\s\S]*?```/g,"").trim().split(`
`)[0].trim();return e.length>50?e.slice(0,47)+"\u2026":e||"New chat"}var K=class{constructor(e){this.storage=e;this.sessions=[];this.activeId="";this.load()}load(){let e=this.storage.get(z,[]);this.sessions=Array.isArray(e)?e:[],this.activeId=this.storage.get(Y,""),this.sessions.find(t=>t.id===this.activeId)||(this.activeId=this.sessions[0]?.id??""),this.sessions.length||this.createSession()}async persist(){await this.storage.update(z,this.sessions),await this.storage.update(Y,this.activeId)}get active(){return this.sessions.find(e=>e.id===this.activeId)}get allSessions(){return[...this.sessions]}get openSessions(){return this.sessions.filter(e=>!e.archived).slice(0,ce)}get historyItems(){return[...this.sessions].sort((e,t)=>t.updatedAt-e.updatedAt)}createSession(){let e={id:de(),title:"New chat",messages:[],createdAt:Date.now(),updatedAt:Date.now(),archived:!1};this.sessions.unshift(e),this.activeId=e.id;let t=this.sessions.filter(s=>s.archived);if(t.length>G){let s=new Set(t.slice(0,G).map(a=>a.id));this.sessions=this.sessions.filter(a=>!a.archived||s.has(a.id))}return this.persist(),e}switchTo(e){let t=this.sessions.find(s=>s.id===e);return t?(t.archived&&(t.archived=!1),this.activeId=e,this.persist(),!0):!1}closeSession(e){let t=this.sessions.find(s=>s.id===e);if(t){if(t.archived=!0,this.activeId===e){let s=this.openSessions.filter(a=>a.id!==e);if(this.activeId=s[0]?.id??"",!this.activeId){this.createSession();return}}this.persist()}}renameSession(e,t){let s=this.sessions.find(a=>a.id===e);s&&(s.title=t.trim()||"Chat",this.persist())}addMessage(e,t){let s=this.active;s&&(s.messages.push({role:e,content:t}),s.updatedAt=Date.now(),s.title==="New chat"&&e==="user"&&(s.title=re(t)),this.persist())}updateLastAssistant(e){let t=this.active;if(!t)return;let s=t.messages.at(-1);s?.role==="assistant"&&(s.content=e,t.updatedAt=Date.now(),this.persist())}clearMessages(){let e=this.active;e&&(e.messages=[],e.title="New chat",e.updatedAt=Date.now(),this.persist())}clearAllHistory(){this.sessions=[],this.activeId="",this.createSession()}};var W={openrouter:["meta-llama/llama-3.3-70b-instruct:free","deepseek/deepseek-r1:free","deepseek/deepseek-chat-v3-0324:free","google/gemma-3-27b-it:free","microsoft/phi-4-reasoning:free","qwen/qwen3-30b-a3b:free","qwen/qwen3-14b:free","mistralai/mistral-7b-instruct:free","nousresearch/deephermes-3-llama-3-8b-preview:free"],huggingface:["Qwen/Qwen2.5-Coder-32B-Instruct","meta-llama/Llama-3.3-70B-Instruct","deepseek-ai/DeepSeek-R1","Qwen/Qwen2.5-72B-Instruct","google/gemma-3-27b-it","mistralai/Mistral-7B-Instruct-v0.3","HuggingFaceH4/zephyr-7b-beta"],groq:["llama-3.3-70b-versatile","deepseek-r1-distill-llama-70b","llama-3.1-8b-instant","gemma2-9b-it","mixtral-8x7b-32768","llama3-70b-8192","llama3-8b-8192"]},$={openrouter:"meta-llama/llama-3.3-70b-instruct:free",huggingface:"Qwen/Qwen2.5-Coder-32B-Instruct",groq:"llama-3.3-70b-versatile"};function le(o,e){switch(o){case"openrouter":return L.openrouter??"https://openrouter.ai/api/v1/chat/completions";case"groq":return L.groq??"https://api.groq.com/openai/v1/chat/completions";case"huggingface":return L.huggingface??"https://router.huggingface.co/v1/chat/completions"}}function pe(o,e){let t={"Content-Type":"application/json"};switch(o){case"openrouter":return{...t,Authorization:`Bearer ${e}`,"HTTP-Referer":"https://github.com/NolvusMadeIt/cascade","X-Title":"Cascade AI"};case"huggingface":return{...t,Authorization:`Bearer ${e}`};case"groq":return{...t,Authorization:`Bearer ${e}`}}}async function*j(o,e,t,s,a,n,u,l){let w=le(o,t),f=pe(o,e),p=await fetch(w,{method:"POST",headers:f,body:JSON.stringify({model:t,messages:s,stream:!0,temperature:a,max_tokens:n,top_p:u}),signal:l});if(!p.ok){let T=await p.text().catch(()=>p.statusText);throw new Error(`${o} ${p.status}: ${T.slice(0,200)}`)}if(!p.body)throw new Error("No response body from provider");let A=p.body.getReader(),D=new TextDecoder,C="";for(;;){let{done:T,value:U}=await A.read();if(T)break;C+=D.decode(U,{stream:!0});let M=C.split(`
`);C=M.pop()??"";for(let I of M){let r=I.trim();if(!r.startsWith("data:"))continue;let v=r.slice(5).trim();if(v==="[DONE]")return;try{let g=JSON.parse(v).choices?.[0]?.delta?.content;g&&(yield g)}catch{}}}}async function ve(o,e){try{let t=await fetch("https://openrouter.ai/api/v1/models",{headers:{Authorization:`Bearer ${o}`}});if(!t.ok)return W.openrouter;let a=(await t.json()).data??[],u=(e?a.filter(l=>l.id.endsWith(":free")||l.pricing?.prompt==="0"):a).map(l=>l.id).sort();return u.length?u:W.openrouter}catch{return W.openrouter}}function X(){let o="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let t=0;t<32;t++)o+=e.charAt(Math.floor(Math.random()*e.length));return o}function Q(o){return o.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function c(o,e){return i.workspace.getConfiguration("cascade").get(o,e)}async function b(o,e){await i.workspace.getConfiguration("cascade").update(o,e,i.ConfigurationTarget.Global)}var L={},ue="https://raw.githubusercontent.com/NolvusMadeIt/cascade/main/endpoints.json",he="https://api.github.com/repos/NolvusMadeIt/cascade/releases/latest";async function ge(o){try{let e=await fetch(ue,{signal:AbortSignal.timeout(8e3)});if(!e.ok)return;let t=await e.json(),s=o.globalState.get("cascade.endpoints",{}),a=Object.keys(s).length===0;L={...t},await o.globalState.update("cascade.endpoints",t),a||Object.entries(t).some(([u,l])=>s[u]!==l)&&await i.window.showInformationMessage("Cascade: Provider endpoints have been updated. Reload VS Code to apply the changes.","Reload Now","Later")==="Reload Now"&&await i.commands.executeCommand("workbench.action.reloadWindow")}catch{}}async function fe(o){try{let e=await fetch(he,{headers:{"User-Agent":"cascade-vscode"},signal:AbortSignal.timeout(8e3)});if(!e.ok)return;let t=await e.json(),s=t.tag_name.replace(/^v/,""),a=o.extension.packageJSON.version;if(s===a)return;let n=f=>f.split(".").map(Number).reduce((k,p,A)=>k+p*1e3**(2-A),0);if(n(s)<=n(a))return;let u=t.assets.find(f=>f.name.endsWith(".vsix"));if(!u||await i.window.showInformationMessage(`Cascade v${s} is available (you have v${a}). Install the update now?`,"Download & Install","Later")!=="Download & Install")return;await i.window.withProgress({location:i.ProgressLocation.Notification,title:"Cascade: Downloading update\u2026",cancellable:!1},async()=>{let f=await fetch(u.browser_download_url,{signal:AbortSignal.timeout(12e4)});if(!f.ok)throw new Error("Download failed");let k=await f.arrayBuffer();await o.globalStorageUri&&i.workspace.fs.createDirectory(o.globalStorageUri).catch(()=>{});let p=i.Uri.joinPath(o.globalStorageUri,`cascade-${s}.vsix`);await i.workspace.fs.writeFile(p,new Uint8Array(k)),await i.commands.executeCommand("workbench.extensions.installExtension",p)}),await i.window.showInformationMessage(`Cascade v${s} installed! Reload VS Code to activate it.`,"Reload Now","Later")==="Reload Now"&&await i.commands.executeCommand("workbench.action.reloadWindow")}catch{}}function me(o){let e=[],t=/```([ \t]*[\w+.-]*)[ \t]*\r?\n([\s\S]*?)```/g,s=/(?:\/\/|#|<!-{2,})\s*[Ff]ile:\s*([^\r\n\-]+?)(?:\s*-{2,}>)?\s*$/,a;for(;(a=t.exec(o))!==null;){let n=a[1].trim(),l=a[2].split(/\r?\n/),w="",f=0;for(let p=0;p<Math.min(5,l.length);p++){let A=s.exec(l[p].trim());if(A){w=A[1].trim(),f=p+1;break}}if(!w)continue;let k=l.slice(f).join(`
`).trimEnd();k.length>20&&e.push({name:w,lang:n,code:k})}return e}var q=[],V=class{resolveWebviewView(e){this.view=e,e.webview.options={enableScripts:!0},this.render([]),q.push(this),e.onDidDispose(()=>{let t=q.indexOf(this);t!==-1&&q.splice(t,1)})}render(e){if(!this.view)return;let t=X(),s=e.length===0,a=e.map((n,u)=>`<div class="tp-item ${n.done?"done":"active"}"><span class="tp-dot">${n.done?"\u2713":u+1}</span><span class="tp-text">${n.text}</span></div>`).join("");this.view.webview.html=`<!DOCTYPE html>
<html><head>
<meta charset="UTF-8"/>
...
</body></html>`}update(e){this.render(e)}};function J(o){for(let e of q)e.update(o)}function be(o){let e=new N(o),t=new N(o),s=new N(o);o.subscriptions.push(i.window.registerWebviewViewProvider("cascade.chat",e,{webviewOptions:{retainContextWhenHidden:!0}}),i.window.registerWebviewViewProvider("cascade.panel",t,{webviewOptions:{retainContextWhenHidden:!0}}),i.window.registerWebviewViewProvider("cascade.secondary",s,{webviewOptions:{retainContextWhenHidden:!0}}),i.window.registerWebviewViewProvider("cascade.tasks",new V,{webviewOptions:{retainContextWhenHidden:!0}}),i.commands.registerCommand("cascade.newChat",()=>e.newChat()),i.commands.registerCommand("cascade.openSettings",()=>e.openSettingsCmd()),i.commands.registerCommand("cascade.clearHistory",()=>e.clearHistory()),i.commands.registerCommand("cascade.focusSidebar",()=>i.commands.executeCommand("cascade.chat.focus")),i.commands.registerCommand("cascade.focusPanel",()=>i.commands.executeCommand("cascade.panel.focus")),i.commands.registerCommand("cascade.focusSecondary",()=>i.commands.executeCommand("cascade.secondary.focus")),i.commands.registerCommand("cascade.showTasks",()=>i.commands.executeCommand("cascade.tasks.focus"))),setTimeout(()=>{ge(o),fe(o)},3e3)}function we(){}var N=class{constructor(e){this.ctx=e;this.attachments=[];this.pendingFiles=new Map;this.sessions=new K(e.globalState)}resolveWebviewView(e){this.view=e,e.webview.options={enableScripts:!0,localResourceRoots:[this.ctx.extensionUri]},e.webview.html=this.getHtml(e.webview),e.webview.onDidReceiveMessage(t=>this.onMessage(t)),e.onDidChangeVisibility(()=>{e.visible&&this.pushSessionState()})}getHtml(e){let t=X(),s=e.asWebviewUri(i.Uri.joinPath(this.ctx.extensionUri,"dist","chat.js")),a=e.asWebviewUri(i.Uri.joinPath(this.ctx.extensionUri,"media","chat.css")),n=e.asWebviewUri(i.Uri.joinPath(this.ctx.extensionUri,"media","cascade-icon.svg")),u=e.asWebviewUri(i.Uri.joinPath(this.ctx.extensionUri,"media","cascade-logo.svg")),l=c("provider","openrouter"),w=c("model",$[l]);return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
    ...
    <div class="cd-msgs-wrap">
      ...
      <div id="empty" class="cd-empty" style="display:flex">
        <div class="cd-empty-inner">
          <img class="cd-empty-icon" src="${u}" alt=""/>
...
</html>`}async onMessage(e){switch(e.type){case"ready":this.pushSessionState(),this.pushKeyStatus(),this.pushModels();break;case"send":await this.handleSend(String(e.text??""),String(e.mode??"ask"));break;case"newSession":this.sessions.createSession(),this.pushSessionState(),this.pushThread();break;case"switchSession":this.sessions.switchTo(String(e.id??"")),this.pushSessionState(),this.pushThread();break;case"closeSession":this.sessions.closeSession(String(e.id??"")),this.pushSessionState(),this.pushThread();break;case"getSettings":await this.pushSettings(),this.pushStats();break;case"saveSettings":await this.saveSettings(e),this.pushKeyStatus(),this.pushModels();break;case"logout":await this.logout();break;case"clearAllHistory":this.sessions.clearAllHistory(),this.pushSessionState(),this.pushThread(),i.window.showInformationMessage("Cascade: All chat history deleted.");break;case"getModels":await this.pushModels();break;case"setProvider":{let t=String(e.provider??"openrouter");await b("provider",t),await b("model",$[t]),this.post({type:"providerChanged",provider:t}),await this.pushModels();break}case"setModel":await b("model",String(e.model??""));break;case"getHistory":this.pushHistory();break;case"getStats":this.pushStats();break;case"restoreSession":this.sessions.switchTo(String(e.id??"")),this.pushSessionState(),this.pushThread();break;case"attachActiveFile":await this.attachActive();break;case"pickWorkspaceFile":await this.pickWorkspaceFile();break;case"pickLocalFile":await this.pickLocalFile();break;case"pickOpenEditor":await this.pickOpenEditor();break;case"attachProblems":await this.attachProblems();break;case"removeAttachment":{let t=String(e.id??"");this.attachments=this.attachments.filter(s=>s.id!==t),this.pushAttachments();break}case"applyFile":await this.applyFileEdit(String(e.code??""),String(e.language??""),e.suggestedPath);break;case"createSuggestedFile":await this.createSuggestedFile(String(e.name??""));break;case"runInTerminal":await this.runInTerminal(String(e.command??""));break;case"openBrowser":await i.commands.executeCommand("simpleBrowser.show","https://openrouter.ai/models?order=newest&supported_parameters=free");break}}async handleSend(e,t){if(!e.trim())return;let s=c("provider","openrouter"),a=c("model",$[s]),n=c("temperature",.2),u=c("maxTokens",4096),l=c("topP",1),w=c("userName",""),f=c("systemPrompt",""),k=c("contextLength",20),p=i.workspace.workspaceFolders?.[0],A=p?p.uri.fsPath:null,D=[`You are Cascade, a friendly and expert AI coding assistant built into VS Code.${w?` The user's name is ${w}.`:""}${A?` The current workspace folder is: ${A}.`:""} Be warm, direct, and encouraging \u2014 like a skilled teammate who genuinely wants to help.`,"Write in a natural, conversational tone. Be concise but thorough. Use 'I' naturally. Briefly acknowledge the user's context before diving in.",'CRITICAL \u2014 File creation rule: Whenever you create or modify a file, you MUST write the COMPLETE file content (never truncate or use placeholders like "rest of code here"). Put the file path as a comment on the VERY FIRST LINE inside the fenced code block \u2014 before any other content. Match the comment style to the language: HTML \u2192 `<!-- File: index.html -->`, JS/TS \u2192 `// File: app.ts`, Python \u2192 `# File: main.py`, CSS \u2192 `/* File: style.css */`. Use a sensible relative path. This allows Cascade to automatically save the file to the workspace.',`For multi-step tasks, open your response with a <steps> block (max 5 items, plain text, one per line):
<steps>
Analyse the request
Write the solution
Explain key decisions
</steps>
Then continue with your full response.`];f&&D.push(f);let C=e;this.attachments.length&&(C=`${this.attachments.map(y=>`<attachment name="${y.label}">
${y.content}
</attachment>`).join(`
`)}

${e}`);let T=this.sessions.active;if(!T)return;if(c("fileAccess","readwrite")!=="none"&&p){let g=p.uri.fsPath,y=i.workspace.textDocuments.filter(h=>!h.isUntitled&&h.uri.scheme==="file"&&h.uri.fsPath.startsWith(g)&&h.getText().length>0&&h.getText().length<8e4);y.length&&(C=`<workspace_context>
${y.map(m=>`<file path="${m.uri.fsPath.slice(g.length).replace(/\\/g,"/").replace(/^\//,"")}">
${m.getText()}
</file>`).join(`
`)}
</workspace_context>

${C}`)}let M=await this.getKey(s);if(!M){this.post({type:"assistantError",text:`No API key for ${s}. Open \u2699 Settings to add your free key.`});return}this.sessions.addMessage("user",e);let I=T.messages.slice(0,-1).slice(-(k*2)).map(g=>({role:g.role,content:g.content})),r=[{role:"system",content:D.join(`
`)},...I,{role:"user",content:C}];this.abortCtrl?.abort(),this.abortCtrl=new AbortController,this.post({type:"assistantStart"});let v="",R=!1;try{for await(let y of j(s,M,a,r,n,u,l,this.abortCtrl.signal)){if(v+=y,!R&&v.includes("</steps>")){let h=/<steps>([\s\S]*?)<\/steps>/.exec(v);if(h){let m=h[1].trim().split(`
`).map(S=>S.trim()).filter(S=>S.length>0);m.length&&(this.post({type:"progressSteps",steps:m}),J(m.map((S,x)=>({text:S,done:!1})))),R=!0}}this.post({type:"assistantDelta",text:v})}this.sessions.addMessage("assistant",v),this.post({type:"progressDone"}),this.post({type:"assistantDone",text:v}),J([]),this.pushSessionState();let g=me(v);if(g.length){let y=i.workspace.workspaceFolders?.[0]?.uri;if(y){let h=[],m;for(let S of g)try{let x=S.name.replace(/\\/g,"/").split("/"),d=i.Uri.joinPath(y,...x);x.length>1&&await i.workspace.fs.createDirectory(i.Uri.joinPath(y,...x.slice(0,-1))).catch(()=>{}),await i.workspace.fs.writeFile(d,Buffer.from(S.code,"utf8")),h.push(S.name),m||(m=d)}catch(x){let d=x instanceof Error?x.message:String(x);i.window.showErrorMessage(`Cascade: Failed to write "${S.name}": ${d}`)}h.length&&(m&&i.window.showTextDocument(m,{preview:!1}),i.window.showInformationMessage(`Cascade wrote ${h.length} file${h.length>1?"s":""}: ${h.join(", ")}`),this.post({type:"filesAutoCreated",files:h}))}else{this.pendingFiles.clear();for(let h of g)this.pendingFiles.set(h.name,{code:h.code,lang:h.lang});this.post({type:"suggestFileCreate",files:g.map(h=>({name:h.name}))})}}}catch(g){if(g.name==="AbortError"){this.post({type:"assistantAbort"});return}let y=g instanceof Error?g.message:String(g);if(/\b(429|503|529)\b/.test(y)||/rate.limit|too many req|overload|temporarily unavail/i.test(y)){let m=c("fallbackProvider","none"),S=m!=="none"?await this.getKey(m):void 0;if(m!=="none"&&S){this.post({type:"rateLimitMsg",text:`\u26A1 ${s} rate limited \u2014 retrying with ${m}\u2026`,countdown:0});try{let d="",P=!1;this.abortCtrl=new AbortController;for await(let B of j(m,S,$[m],r,n,u,l,this.abortCtrl.signal)){if(d+=B,!P&&d.includes("</steps>")){let E=/<steps>([\s\S]*?)<\/steps>/.exec(d);if(E){let O=E[1].trim().split(`
`).map(F=>F.trim()).filter(F=>F.length>0);O.length&&this.post({type:"progressSteps",steps:O}),P=!0}}this.post({type:"assistantDelta",text:d})}this.sessions.addMessage("assistant",d),this.post({type:"progressDone"}),this.post({type:"assistantDone",text:d}),this.pushSessionState()}catch(d){let P=d instanceof Error?d.message:String(d);this.post({type:"assistantError",text:`Fallback (${m}) also failed: ${P}`})}return}let x=30;this.post({type:"rateLimitMsg",text:`\u23F3 ${s} rate limited (free tier). Retrying in ${x}s\u2026`,countdown:x});for(let d=x-1;d>0;d--){if(await new Promise(P=>setTimeout(P,1e3)),this.abortCtrl?.signal.aborted){this.post({type:"assistantAbort"});return}this.post({type:"rateLimitMsg",text:`\u23F3 Rate limited \u2014 retrying in ${d}s\u2026`,countdown:d})}if(await new Promise(d=>setTimeout(d,1e3)),this.abortCtrl?.signal.aborted){this.post({type:"assistantAbort"});return}this.post({type:"rateLimitMsg",text:"\u{1F504} Retrying\u2026",countdown:0});try{let d="",P=!1;this.abortCtrl=new AbortController;for await(let B of j(s,M,a,r,n,u,l,this.abortCtrl.signal)){if(d+=B,!P&&d.includes("</steps>")){let E=/<steps>([\s\S]*?)<\/steps>/.exec(d);if(E){let O=E[1].trim().split(`
`).map(F=>F.trim()).filter(F=>F.length>0);O.length&&this.post({type:"progressSteps",steps:O}),P=!0}}this.post({type:"assistantDelta",text:d})}this.sessions.addMessage("assistant",d),this.post({type:"progressDone"}),this.post({type:"assistantDone",text:d}),this.pushSessionState()}catch(d){let P=d instanceof Error?d.message:String(d);this.post({type:"assistantError",text:`Still rate limited after retry. Try a different model or provider.

${P.slice(0,120)}`})}return}this.post({type:"assistantError",text:y})}}async pushSettings(){let e=!!await this.ctx.secrets.get("cascade.orKey"),t=!!await this.ctx.secrets.get("cascade.hfKey"),s=!!await this.ctx.secrets.get("cascade.groqKey");this.post({type:"settingsForm",hasOrKey:e,hasHfKey:t,hasGroqKey:s,temperature:c("temperature",.2),maxTokens:c("maxTokens",4096),topP:c("topP",1),contextLength:c("contextLength",20),fileAccess:c("fileAccess","none"),fileScope:c("fileScope","workspace"),approvalMode:c("approvalMode","ask"),terminalAccess:c("terminalAccess",!1),gitAccess:c("gitAccess",!1),openRouterFreeOnly:c("openRouterFreeOnly",!0),fallbackProvider:c("fallbackProvider","none"),userName:c("userName",""),systemPrompt:c("systemPrompt",""),historyRetention:c("historyRetention","unlimited"),autoHideCode:c("autoHideCode",!0)})}async saveSettings(e){e.openRouterKey&&await this.ctx.secrets.store("cascade.orKey",String(e.openRouterKey)),e.huggingfaceKey&&await this.ctx.secrets.store("cascade.hfKey",String(e.huggingfaceKey)),e.groqKey&&await this.ctx.secrets.store("cascade.groqKey",String(e.groqKey)),e.temperature!==void 0&&await b("temperature",Number(e.temperature)),e.maxTokens!==void 0&&await b("maxTokens",Number(e.maxTokens)),e.topP!==void 0&&await b("topP",Number(e.topP)),e.contextLength!==void 0&&await b("contextLength",Number(e.contextLength)),e.fileAccess!==void 0&&await b("fileAccess",String(e.fileAccess)),e.fileScope!==void 0&&await b("fileScope",String(e.fileScope)),e.approvalMode!==void 0&&await b("approvalMode",String(e.approvalMode)),e.terminalAccess!==void 0&&await b("terminalAccess",!!e.terminalAccess),e.gitAccess!==void 0&&await b("gitAccess",!!e.gitAccess),e.openRouterFreeOnly!==void 0&&await b("openRouterFreeOnly",!!e.openRouterFreeOnly),e.fallbackProvider!==void 0&&await b("fallbackProvider",String(e.fallbackProvider)),e.userName!==void 0&&await b("userName",String(e.userName)),e.systemPrompt!==void 0&&await b("systemPrompt",String(e.systemPrompt)),e.historyRetention!==void 0&&await b("historyRetention",String(e.historyRetention)),e.autoHideCode!==void 0&&await b("autoHideCode",!!e.autoHideCode),e.autoHideCode!==void 0&&await b("autoHideCode",!!e.autoHideCode)}async logout(){await this.ctx.secrets.delete("cascade.orKey"),await this.ctx.secrets.delete("cascade.hfKey"),await this.ctx.secrets.delete("cascade.groqKey"),i.window.showInformationMessage("Cascade: All API keys cleared."),await this.pushSettings()}async getKey(e){let t={openrouter:"cascade.orKey",huggingface:"cascade.hfKey",groq:"cascade.groqKey"};return this.ctx.secrets.get(t[e])}async pushKeyStatus(){let e=!!await this.ctx.secrets.get("cascade.orKey"),t=!!await this.ctx.secrets.get("cascade.hfKey"),s=!!await this.ctx.secrets.get("cascade.groqKey");this.post({type:"keyStatus",hasOrKey:e,hasHfKey:t,hasGroqKey:s})}async pushModels(){let e=c("provider","openrouter"),t=c("openRouterFreeOnly",!0),s=c("model",$[e]),a=await this.getKey(e);if(!a){this.post({type:"models",models:[],selectedModel:"",noKey:!0});return}let n;e==="openrouter"?n=await ve(a,t):n=[...W[e]],n.includes(s)||n.unshift(s),this.post({type:"models",models:n,selectedModel:s,noKey:!1})}pushSessionState(){let e=this.sessions.openSessions;this.post({type:"sessionState",activeSessionId:this.sessions.active?.id??"",sessions:e.map(t=>({id:t.id,title:t.title}))})}pushThread(){let e=this.sessions.active;this.post({type:"loadThread",messages:e?.messages??[]}),this.attachments=[],this.pushAttachments()}pushStats(){let e=this.sessions.allSessions,t=e.reduce((r,v)=>r+v.messages.length,0),s=new Set,a={};for(let r of e){for(let R of r.messages);let v=new Date(r.updatedAt).toISOString().slice(0,10);s.add(v),a[v]=(a[v]??0)+r.messages.length}for(let r of e){let v=new Date(r.createdAt).toISOString().slice(0,10);s.add(v),a[v]||(a[v]=0)}let n=[...s].sort(),u=0,l=0,w=0,f=new Date().toISOString().slice(0,10),k=new Date(Date.now()-864e5).toISOString().slice(0,10);for(let r=0;r<n.length;r++){let v=r>0?n[r-1]:"",R=n[r];w=(v?(new Date(R).getTime()-new Date(v).getTime())/864e5:1)===1?w+1:1,w>l&&(l=w)}if(s.has(f)||s.has(k)){let r=s.has(f)?f:k;for(;s.has(r);)u++,r=new Date(new Date(r).getTime()-864e5).toISOString().slice(0,10)}let p=[0,0,0,0,0,0,0];for(let r of n)p[new Date(r).getDay()]++;let D=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][p.indexOf(Math.max(...p))],C=[],T=new Date(Date.now()-83*864e5);T.setHours(0,0,0,0);for(let r=0;r<84;r++){let v=new Date(T.getTime()+r*864e5).toISOString().slice(0,10);C.push(a[v]??0)}let U=c("provider","openrouter"),M=c("model",""),I=this.ctx.extension.packageJSON.version;this.post({type:"statsData",sessions:e.length,messages:t,activeDays:s.size,streak:u,longestStreak:l,peakDay:D,gridData:C,provider:U,model:M,version:I})}pushHistory(){let e=this.sessions.historyItems.map(t=>({id:t.id,title:t.title,preview:t.messages.find(s=>s.role==="user")?.content.slice(0,80)??"",updatedAt:t.updatedAt,archived:t.archived,messageCount:t.messages.length}));this.post({type:"historyState",activeSessionId:this.sessions.active?.id??"",items:e})}pushAttachments(){this.post({type:"attachmentsUpdated",items:this.attachments.map(e=>({id:e.id,label:e.label}))})}async attachActive(){let e=i.window.activeTextEditor?.document;if(!e){i.window.showWarningMessage("Cascade: No active file to attach.");return}if(c("fileAccess","none")==="none"){await i.window.showWarningMessage("Cascade: File access is disabled.","Open Settings")&&this.post({type:"settingsForm"});return}this.addAttachment(e.fileName.split(/[\\/]/).pop()??"file",e.getText())}async pickWorkspaceFile(){let e=await i.window.showOpenDialog({canSelectMany:!1,openLabel:"Attach"});if(!e?.length)return;let t=await i.workspace.fs.readFile(e[0]),s=Buffer.from(t).toString("utf8");this.addAttachment(e[0].fsPath.split(/[\\/]/).pop()??"file",s)}async pickLocalFile(){let e=await i.window.showOpenDialog({canSelectMany:!1,openLabel:"Upload"});if(!e?.length)return;let t=await i.workspace.fs.readFile(e[0]),s=Buffer.from(t).toString("utf8");this.addAttachment(e[0].fsPath.split(/[\\/]/).pop()??"file",s)}async pickOpenEditor(){let e=i.workspace.textDocuments.filter(s=>!s.isUntitled&&s.uri.scheme==="file");if(!e.length){i.window.showWarningMessage("No open editors to attach.");return}let t=await i.window.showQuickPick(e.map(s=>({label:s.fileName.split(/[\\/]/).pop()??s.fileName,description:s.fileName,doc:s})));t&&this.addAttachment(t.label,t.doc.getText())}async attachProblems(){let e=i.window.activeTextEditor?.document,t=e?i.languages.getDiagnostics(e.uri):i.languages.getDiagnostics().flatMap(([,a])=>a);if(!t.length){i.window.showInformationMessage("No problems to attach.");return}let s=t.map(a=>`[${i.DiagnosticSeverity[a.severity]}] Line ${a.range.start.line+1}: ${a.message}`).join(`
`);this.addAttachment("Problems",s)}addAttachment(e,t){let s=Date.now().toString(36);this.attachments.push({id:s,label:e,content:t}),this.pushAttachments()}async createSuggestedFile(e){let t=this.pendingFiles.get(e);if(!t){i.window.showWarningMessage(`Cascade: Could not find code for "${e}". Try clicking Apply to File on the code block.`);return}let s=i.workspace.workspaceFolders?.[0]?.uri;if(!s){await this.applyFileEdit(t.code,t.lang,e);return}try{let a=i.Uri.joinPath(s,e),n=e.split("/");if(n.length>1){let u=i.Uri.joinPath(s,...n.slice(0,-1));await i.workspace.fs.createDirectory(u).catch(()=>{})}await i.workspace.fs.writeFile(a,Buffer.from(t.code,"utf8")),await i.window.showTextDocument(a),this.post({type:"fileCreated",name:e})}catch(a){let n=a instanceof Error?a.message:String(a);i.window.showErrorMessage(`Cascade: Failed to create "${e}": ${n}`)}}async applyFileEdit(e,t,s){if(c("fileAccess","none")!=="readwrite"){i.window.showWarningMessage("Cascade: File write access is disabled. Enable it in Settings.");return}let n=await i.window.showSaveDialog({defaultUri:s?i.Uri.joinPath(i.workspace.workspaceFolders?.[0]?.uri??i.Uri.file("/"),s):void 0,saveLabel:"Apply Code"});n&&(await i.workspace.fs.writeFile(n,Buffer.from(e,"utf8")),i.window.showTextDocument(n))}async runInTerminal(e){if(!c("terminalAccess",!1)){i.window.showWarningMessage("Cascade: Terminal access is disabled. Enable it in Settings.");return}if(c("approvalMode","ask")==="ask"&&await i.window.showWarningMessage(`Run in terminal?
